...
#define RESET_USER_DATA			(1)   //1：开启重置用户数据按钮。0：关闭重置用户数据按钮
	...
	#define RESET_VALID_LEVEL	(0)		//有效电平
		...
		#define RESET_KEY_VAL	(GPIO_ReadInputDataBit(RESET_KEY_PORT, RESET_KEY_PIN)^0x01)
	#else
...
//外部看门狗端口
#define EXTERNAL_WDG_EN		(1u) 		//1：开启外部看门狗。0：不产生外部喂狗
...
#define RF_SWITCH_PIN		GPIO_Pin_0		//无线电源开关引脚
...
#define SIGNAL_ON	STATUS0(SIGNAL_PORT, SIGNAL_PIN)
#define SIGNAL_OFF	STATUS1(SIGNAL_PORT, SIGNAL_PIN)
