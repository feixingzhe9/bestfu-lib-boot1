...
static BF_INT08U SI4432SendEn;			//发送使能标识，2014.10.08 Unarty Add
#ifndef SI4432_THRESHOLD
	static BF_INT08U Threshold = 160;	//主机门槛值
#endif 
//周旺添加，测试RSSI变量定义
//Freq_433M_920KHz
const BF_INT08U Si4432Cfg_Tab[21] = {0x2B,0x40,0x0A,0x03,0x68,0x01,0x3A,0x93,0x02,0xF6,
							  0x1D,0x80,0x60,0x4E,0xA5,0x2D,0x23,0x0D,0x53,0x62,0x00};

//Private function declaration
static BF_INT08U Check_433M_Busy(BF_INT08U level);
static void Si4432_PortInit(void);				  
static BF_INT08U  SPI_RW_Reg(BF_INT08U addr , BF_INT08U data);
static void Si4432_PortInit(void);
static void SetMode_RX(void);
static void SetMode_TX(void);
static void Si4432_Fault(BF_INT08U event);
#ifndef SI4432_THRESHOLD
static BF_INT08U Si4432_Threshold(void);
#endif
...
//信号指示灯
	...
	#define TX		(0x55)
	#define RX		(0xaa)
	static void Si4332_Signal_Insturct(BF_INT08U state);
	...
	*******************************************************************************/
	...
	{
		GPIO_ToggleBits(SIGNAL_PORT, SIGNAL_PIN);
	}
	/*******************************************************************************
...
返 回 值:  通信门槛值
*******************************************************************************/
BF_INT08U  GetThresholdPara(void)
{
#ifndef SI4432_THRESHOLD
	return Threshold ;
#else
...
/*******************************************************************************
函 数 名:  	BF_INT08U SPI_RW_Reg(BF_INT08U addr,BF_INT08U data)
功能说明:  	通过SPI发送两个字节
参    数:  	BF_INT08U addr - 寄存器地址
			BF_INT08U data - 寄存器设定值 			
返 回 值:  	无
*******************************************************************************/
static BF_INT08U SPI_RW_Reg(BF_INT08U addr,BF_INT08U data)
{
...
{
	EXTI->IMR &= ~(nIRQ_PIN);
	GPIO_Config(SDN_PORT, SDN_PIN, OUT_PP_2M);
    GPIO_Config(nSEL_PORT, nSEL_PIN, OUT_PP_2M);
    GPIO_Config(nIRQ_PORT, nIRQ_PIN, IN_FLOATING);
    GPIO_Config(nGPIO2_PORT, nGPIO2_PIN, IN_FLOATING);
    GPIO_SetBits(nIRQ_PORT, nIRQ_PIN);		//上拉
    GPIO_SetBits(nGPIO2_PORT, nGPIO2_PIN);	//上拉
#if SIGNAL == 1
	GPIO_Config(SIGNAL_PORT, SIGNAL_PIN, OUT_PP_2M);
	SIGNAL_OFF;
#endif
#if	RF_SWITCH == 1
	GPIO_Config(RF_SWITCH_PORT, RF_SWITCH_PIN, OUT_PP_2M);
	RF_SWITCH_ON;
#endif
...
//默认433 1M偏移
	...
	EXTI_NVIC_Config(nIRQ_PORT, 10, EXTI_Trigger_Falling); 	//下降沿触发 PB10 - nIRQ
	MY_NVIC_Init(2, 0, EXTI15_10_IRQn, NVIC_PriorityGroup_2);	//抢占2，子优先级0，组2	  
	EXTI->IMR &= ~(nIRQ_PIN);
...
返 回 值:   无
*******************************************************************************/
static void SI4432_Send_All_Data(BF_INT08U *cptBuf, BF_INT08U sendLength, BF_INT16U uiOvertime)
{
	BF_INT32U i, t = 0; 
...
{
	BF_INT08U resVal1,i = 0,fifo_success = TRUE; //接收队列溢出标志
	BF_INT08U data[64]; //433接收缓存
	static BF_INT08U seat = 0;
	...
	{ 
		...
		resVal1 = SPI_RW_Reg(R_REG | 0x03,0x00);	//读中断寄存器1
				  SPI_RW_Reg(R_REG | 0x04,0x00);  //读中断寄存器2
...
参    数:   BF_INT08U *cptBuf - 将要发送数据包指针
            BF_INT08U cLength - 将要发送数据包长度
		    BF_INT16U uiOvertime - 将要发送数据最大延时  单位：秒
		    RxTxStatus_btst *btstFlag - 发送标志
返 回 值:  	0 - 在设定时间内，未发送成功
			1 - 在设定时间内，发送成功
*******************************************************************************/
BF_BOOLEAN Si4432_Send(BF_INT08U *cptBuf, BF_INT08U cLength, BF_INT08U level)             
{
...
/*******************************************************************************
函 数 名:  BF_INT08U Check_433M_Busy(void)
功能说明:  检测433M信道空闲状态
参    数:  	level:	随机等待基数
返 回 值:  	1 - BUSY	忙碌
			0 - SPARE	空闲
...
static BF_INT08U Check_433M_Busy(BF_INT08U level)
{
	const static BF_INT16U randWait_Tab[] = {5, 5, 5, 5};
	static BF_INT16U busyTime = 0;		//连续检测忙的时间
	static BF_INT16U idleTime = 0;		//连续检测空闲时间		
	static BF_INT16U rand = 0;			//数据发送随机等待时间
...
static BF_INT08U Si4432_Threshold(void)
{
	static BF_INT08U buf[4] = {0};
	static BF_INT32U time = 0;	//计算时间值
	static BF_INT32U calculTime = 0;	 //上次计算时间
	BF_INT08U i;
